...
If successful, it's true that their will inevitably be developers trying to exploit the agent facing APIs. However, common attack patters like DDoS are immediately far less attainable with usage based pricing. As the pricing finds a dynamic equilibrium, these types of exploits are far too expensive for most attackers.
